...
export default async function VehiclePage({
  ...
  // RLS returns no row if vehicle doesn't belong to this user.
  if (!vehicle) notFound();
  ...
  return (
    <main className="flex flex-1 flex-col">
      ...
      <div className="mx-auto w-full max-w-lg flex-1 px-4 py-6">
        ...
        {/* Stats — placeholder, wired in slice 4 */}
        <div className="mt-4 grid grid-cols-3 gap-2">
          {[
            { label: "Current mileage", value: "—" },
            { label: "Avg MPG", value: "—" },
            { label: "Total spent", value: "—" },
          ].map(({ label, value }) => (
            <div
              key={label}
              className="flex flex-col items-center rounded-xl border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-900/50"
            >
